...
<html lang="en">
  ...
  <!--<![endif]-->
    ...
    <div class="content">
        <div class="header">
            <span class="page-title">添加订单</span>
        </div>
        ...
        <div class="container-fluid">
            <div class="row-fluid">
				...
				<div  style="color:red;">
					...
					<c:forEach items="${error.messages}" var="err">
						${err}
					</c:forEach>
				</div>
				<form id="ff" action="${ctx}jsp/addOrderInfo.do" method="post">
			        <label>城市</label>
			        <input type="text" name="cityName" value="${orderInfo.cityName}" class="input-xlarge">
			        <label>手机号</label>
			        <input type="text" name="mobile" value="${orderInfo.mobile}" class="input-xlarge">
			        <label>地址</label>
			        <textarea name="address" value="" rows="3" class="input-xlarge">${orderInfo.address}</textarea>
			    </form>
			    </div>
    <button class="btn btn-primary" onclick="save()"><i class="icon-save"></i> 保存</button>
    <a href="${ctx}jsp/orderInfoManager.do" class="btn">取消</a>
				 <footer>
				     <hr>
				     <p>&copy; 2016</p>
				 </footer>
