            ...
            wrapBehavior : function(self, action) {
                return self['__bui_wrap_' + action] = function (e) {
                    if (!self.get('disabled')) {
                        var ret = self[action](e);
                        // thc,解决Editor按回车窗体消失BUG
                        if(ret === false) {
	                       	return false;
                       	}
                    }
  ...
  keyNav.prototype = {
    ...
    _handleKeyDown : function(ev){
      ...
      switch(code){
        case KeyCode.UP :
          ev.preventDefault();
          ret = _self.handleNavUp(ev);
          break;
        case KeyCode.DOWN : 
          ev.preventDefault();
          ret = _self.handleNavDown(ev);
          break;
        case KeyCode.RIGHT : 
          ev.preventDefault();
          ret = _self.handleNavRight(ev);
          break;
        case KeyCode.LEFT : 
          ev.preventDefault();
          ret = _self.handleNavLeft(ev);
          break;
        case KeyCode.ENTER : 
          ret = _self.handleNavEnter(ev);
          break;
        case KeyCode.ESC : 
          ret = _self.handleNavEsc(ev);
          break;
        case KeyCode.TAB :
          ret = _self.handleNavTab(ev);
          break;
        default:
          break;
      }
...

  {
    ...
    sortInfo: {
      getter : function(){
		...
		// thc
		var info = {};
		info[_self.get('sortProperty') || 'field'] = field;
		info[_self.get('directionProperty') || 'direction'] =  _self.get('sortDirection');
...
define('bui/data/store',['bui/data/proxy','bui/data/abstractstore','bui/data/sortable'],function(require) {
  ...
  store.ATTRS = 
  {
     ...
     *     errorProperty : 'errorMsg', //存放错误信息的字段(error)
     *     hasErrorProperty : 'isError', //是否错误的字段（hasError)
     *     hasSuccessProperty : 'success', //是否正确的字段（success),等同于!hasErrorProperty
     *     root : 'data',               //存放数据的字段名(rows)
     *     totalProperty : 'total'     //存放记录总数的字段名(results)
     *   });
     * </code></pre>
...
define('bui/grid/grid',['bui/common','bui/mask','bui/toolbar','bui/list','bui/grid/header','bui/grid/column'],function (require) {
  ...
  var grid = List.SimpleList.extend({
    ...
    _formatBar:function(bar) {
    	if(!bar) {
    		return bar;
    	}
    	bar.items = this._formatBarItems(bar.items);
    	var hasCtrl = false;
    	for(var key in bar) {
    		if(bar[key]) {
    			hasCtrl = true;
    	...
    	return hasCtrl ? bar : false;
    },
    _formatBarItems:function(items) {
